...
{
	private $table_name			= 'user_groups';			// user accounts
	 ...
	 * Can be called for not activated UserGroups only.
	 *
	 * @param	int
	 * @param	string
	 * @param	bool
	...
	/**
	 * Purge table of non-activated UserGroups
	 *
	 * @param	int
	 * @return	void
	 */
	...
	public function getPermissions($userGroupID) {
		...
		//if ($actions === false) {
			$userGroupIDArray= explode(',', $userGroupID);
			$userGroupIDArray = array_map('trim', $userGroupIDArray);
			var_dump($userGroupIDArray);
			if(is_array($userGroupIDArray)) {
				$this->db->where_in("user_group_permissions.user_group_id", $userGroupIDArray);
			} else {
				$this->db->where("user_group_permissions.user_group_id", $userGroupIDArray);
			}
			$this->db->where("user_group_permissions.allowed", 1);
			
			$query = $this->db->get("user_group_permissions");
			if ($query->num_rows() >  0) {
				foreach ($query->result_array() as  $action) {
					$permissions[] = $action['controller'].'/'.$action['action'];
				}
			
				// $actions = $this->UserGroupPermission->find('all',array('conditions'=>array(
				// 'UserGroupPermission.user_group_id' => $userGroupIDArray,
				// 'UserGroupPermission.allowed'=>1)));
...
/* Location: ./application/models/auth/UserGroups.php */